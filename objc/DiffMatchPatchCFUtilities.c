...
Boolean diff_regExMatch(CFStringRef text, const regex_t *re) {
  ...
  if (bytes == NULL) {
    ...
    if (success) {
      ...
      localBuffer = (char *)calloc(length, sizeof(char));
...
CFIndex diff_commonOverlap(CFStringRef text1, CFStringRef text2) {
  ...
  if (text1_length > text2_length) {
    ...
    text2_trunc = (CFStringRef)CFRetain(text2);
  } else if (text1_length < text2_length) {
    text1_trunc_length = text1_length;
    text1_trunc = (CFStringRef)CFRetain(text1);
...
CFArrayRef diff_halfMatchCreate(CFStringRef text1, CFStringRef text2, const float diffTimeout) {
  ...
  if (hm1 == NULL && hm2 == NULL) {
    ...
    hm = (CFArrayRef)CFRetain(hm1);
  } else if (hm1 == NULL) {
    hm = (CFArrayRef)CFRetain(hm2);
  } else {
    // Both matched.  Select the longest.
    hm = CFStringGetLength((CFStringRef)CFArrayGetValueAtIndex(hm1, 4)) > CFStringGetLength((CFStringRef)CFArrayGetValueAtIndex(hm2, 4)) ? (CFArrayRef)CFRetain(hm1) : (CFArrayRef)CFRetain(hm2);
  }
