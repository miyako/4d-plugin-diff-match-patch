...
- (NSString *)diff_prettyHtml:(NSMutableArray *)diffs;
{
  ...
  for (Diff *aDiff in diffs) {
    ...
    switch (aDiff.operation) {
      case DIFF_INSERT:
            [html appendFormat:@"<span style=\"background-color:#e6ffe6;\">%@</span>", text];
//            [html appendFormat:@"<ins style=\"background:#e6ffe6;\">%@</ins>", text];
        break;
      case DIFF_DELETE:
            [html appendFormat:@"<span style=\"background-color:#ffe6e6;\">%@</span>", text];
  ...
  for (Diff *aDiff in diffs) {
    switch (aDiff.operation) {
      case DIFF_INSERT:
        ...
        break;
      case DIFF_DELETE:
        [delta appendFormat:@"-%d\t", (int32_t)aDiff.text.length];
        break;
      case DIFF_EQUAL:
        [delta appendFormat:@"=%d\t", (int32_t)aDiff.text.length];
        break;
