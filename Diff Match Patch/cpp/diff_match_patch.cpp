...
QString diff_match_patch::diff_prettyHtml(const QList<Diff> &diffs, Mode mode) {
  QString html;
  QString text;
  foreach(Diff aDiff, diffs) {
    text = aDiff.text;
    ...
    switch (aDiff.operation) {
      case INSERT:
			switch (mode){
				case SPAN4D:
				html += QString("<span style=\"background-color:#e6ffe6;\">") + text
				+ QString("</span>");
				break;
				default:
        ...
        break;
			...
			switch (mode){
				case SPAN4D:
				html += QString("<span style=\"background-color:#ffe6e6;\">") + text
				+ QString("</span>");
				break;
				default:
